...
export function buildAnalyzePrompt(request) {
  ...
  return [
    ...
    "Set recommendation.verdict from the evidence you found: strong_fit when required areas are evidenced and gaps are cosmetic; worth_applying when the required core is there and the rest is closable before applying; stretch when a required area is genuinely missing but the role is reachable with preparation; weak_fit when several required areas are absent. recommendation.rationale must name the specific requirements that drove the choice.",
    ...
    "Every gap needs howToClose: what the candidate can actually do about it. Prefer what is reachable before applying (reframe existing work, surface a buried project, name adjacent experience honestly) over generic advice like 'get more experience'. Where it truly cannot be closed before applying, say so and give the shortest honest path.",
