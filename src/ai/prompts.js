...
export function buildAnalyzePrompt(request) {
  ...
  return [
    ...
    "Each employer question must be the exact sentence the candidate can send or say, and must state when to raise it — application form, recruiter screen, hiring-manager call, or offer stage. uncertainties[].type is a short topic label the reader sees as a heading — the English idea of \"Team size\" or \"Regulatory scope\", written in the output language like every other analysis field. Never emit a machine token like \"sponsorship\" or \"scope_unclear\".",
