...
export function buildAnalyzePrompt(request) {
  ...
  return [
    ...
    "candidate.workAuthorization and candidate.targetMarket are what the candidate said about themselves, not findings about them. Where a stated condition requires something the candidate's declared status does not obviously provide, treat that requirement as an unmet knockout and say which condition and which declared status you are comparing. Never state or imply that the candidate is or is not legally eligible, never name an immigration rule, route, quota or processing time, and never advise on obtaining status — the comparison you may make is between the employer's own sentence and the candidate's own answer, and the reader must be told to verify it themselves.",
    "Use candidate.targetMarket for two things only: CV conventions in that market — length, whether a photo, date of birth or marital status is normal, how dates and qualifications are written — and pointing out when the posting's location is outside the market the candidate said they want. Do not use it to describe visa or immigration policy.",
